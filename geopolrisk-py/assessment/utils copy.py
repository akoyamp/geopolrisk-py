...
def filter_dataframe(dataframe, column, filterlist):
    """
    Filter a DataFrame based on values in a specific column.

    Parameters:
        df (pandas.DataFrame): The DataFrame to filter.
        column_name (str): The name of the column to filter by.
        filter_values (list): A list of values to filter on.

    Returns:
        pandas.DataFrame: The filtered DataFrame.
    """
    filtered_df = dataframe[dataframe[column].isin(filterlist)]
    return filtered_df


def vfy_arg(expected_type):
    def decorator(func):
