...
class TestGeoPolRiskPy(unittest.TestCase):
    ...
    ABS_TOL = 1e-6
    REL_TOL_PRODQTY = 1e-6
    REL_TOL_SCORE = 1e-2
    REL_TOL_HHI = 1e-2
    REL_TOL_CF = 1e-2

    def assert_close(self, actual, expected, rel_tol, msg):
        self.assertTrue(
            math.isclose(actual, expected, rel_tol=rel_tol, abs_tol=self.ABS_TOL),
            msg=(
                f"{msg}. actual={actual}, expected={expected}, "
                f"rel_tol={rel_tol}, abs_tol={self.ABS_TOL}"
            ),
        )
    @classmethod
    def setUpClass(cls):
        ...
        for rawmaterial in ["Nickel", "Manganese", "Graphite"]:
            ...
            for _, row in df_subset.iterrows():
                ...
                self.assert_close(
                    prodqty,
                    expected_prodqty,
                    self.REL_TOL_PRODQTY,
                    f"ProdQty mismatch for {rawmaterial} {year} {country_code}",
                )
                print(
                ...
                self.assert_close(
                    ...
                    expected_hhi,
                    self.REL_TOL_HHI,
                    f"HHI mismatch for {rawmaterial} {year} {country_code}",
                )
        ...
        for rawmaterial in ["Nickel", "Manganese", "Graphite"]:
            ...
            for _, row in df_subset.iterrows():
                ...
                self.assert_close(
                    ...
                    expected_score,
                    self.REL_TOL_SCORE,
                    f"GeoPolRisk Score mismatch for {rawmaterial} {year} {importer_code}",
                )
                print(
                ...
                self.assert_close(
                    ...
                    expected_cf,
                    self.REL_TOL_CF,
                    f"CF mismatch for {rawmaterial} {year} {importer_code}",
                )
                print(
